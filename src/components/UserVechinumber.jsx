...
<div className="flex justify-center mb-4">
  <button onClick={click} className="bg-green-500 hover:bg-green-600 focus:bg-green-600 text-white px-2 py-1 rounded-md">
    เพิ่มเลขทะเบียนรถ
  </button>
</div>
      {car?.map((item) => (
...
function ReseverdItem({ item }) {
  ...
  const handleDelete = (id) => {
    const isConfirmed = window.confirm("คุณต้องการลบทะเบียนที่ลงไว้แล้วหรือไม่?");
    if (isConfirmed) {
      deletecar(id);
